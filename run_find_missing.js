...
const REMOTE_SCRIPT = '/var/www/poisson-backend/find_missing.js';
const LOCAL_SCRIPT = path.join(__dirname, 'find_missing.js');
...
conn.on('ready', () => {
  conn.sftp((err, sftp) => {
    sftp.fastPut(LOCAL_SCRIPT, REMOTE_SCRIPT, (err) => {
      conn.exec('cd /var/www/poisson-backend && node find_missing.js && rm find_missing.js', (err, stream) => {
