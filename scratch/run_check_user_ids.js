...
conn.on('ready', () => {
    ...
    s.connect({ host: '72.60.254.10', port: 22, username: 'root', password: 'i5dAN0hN.HNAlWaYtS.' }).then(() => {
        return s.put('c:/poisson-backend/scratch/check_user_ids.js', '/var/www/poisson-backend/check_user_ids.js');
    }).then(() => {
        conn.exec('cd /var/www/poisson-backend && node check_user_ids.js', (err, stream) => {
            if (err) throw err;
