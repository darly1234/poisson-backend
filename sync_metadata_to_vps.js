...
conn.on('ready', () => {
    ...
    const tabsStr = JSON.stringify(localMetadata.tabs).replace(/'/g, "''");
    const bankStr = JSON.stringify(localMetadata.fieldbank).replace(/'/g, "''");

    const sql = `UPDATE metadata SET tabs = '${tabsStr}', fieldbank = '${bankStr}', updated_at = NOW() WHERE id = (SELECT id FROM metadata LIMIT 1);`;

    conn.exec(`psql -U postgres -d poisson_erp -c "${sql}"`, (err, stream) => {
        if (err) throw err;
